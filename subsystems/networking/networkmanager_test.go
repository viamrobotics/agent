...
func (bsm *bluetoothServiceMock) waitForCredentials(ctx context.Context, _, _ bool, _ chan<- userInput) error {
	if bsm.shouldFailToWaitForCredentials {
		return errors.New("mock error: fail to wait for credentials")
	}
	return nil
}
