...
The tests should not require any user input while they are running, aside from maybe accepting a macOS dialogue requesting permission to control the network adapter. This test will cause your Mac to connect to the wifi hotspot hosted by the Pi. The captive portal will appear and disappear on its own - you do not need to interact with it.
